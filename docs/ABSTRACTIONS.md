...
- Uses `get_ai_agents_status`, whose backend checks Claude Code, Codex, GitHub Copilot, OpenCode, Pi, Antigravity, Kiro, and Hermes Agent by treating the app process path, login-shell path, and supported local/toolchain/app install locations, including nvm-managed Node installs, Kiro's official `%ProgramFiles%\Kiro-Cli` MSI location, OpenCode Desktop's current `%LOCALAPPDATA%\Programs\@opencode-aidesktop` install root, plus Windows `.exe` and npm/pnpm/Scoop shim paths, as valid CLI-agent sources. Shared Windows PATH lookup skips extensionless npm wrappers in favor of launchable batch/native results. OpenCode additionally honors an explicit `OPENCODE_PATH` binary/directory and npm/pnpm prefix roots because Windows GUI launches often miss the terminal's resolved npm shim path.
...
interface Settings {
  ...
  tray_resident_mode_enabled: boolean | null // null = default false
}
```
...
`tray_resident_mode_enabled` is installation-local and defaults to `false`, so existing installs keep quitting when the main window is closed. When it is on, Tolaria shows a menu bar / system tray icon and the main window hides instead of quitting; see [ADR-0173](adr/0173-opt-in-tray-resident-mode.md).

`src/lib/trayResidentMode.ts` is the renderer-side boundary: it resolves the effective value, invokes `set_tray_resident_mode` and emits `tray_resident_mode_toggled` only when the value actually changes, so `useSettings` stays a persistence hook. Everything else — icon lifecycle, close interception, macOS activation policy — stays in Rust (`tray.rs` glue over the pure rules in `tray_state.rs`), which is why no tray capability appears in `capabilities/default.json`.
...
### CI/CD
- **`.circleci/config.yml`** — Authoritative CI/CD graph for portable validation and native releases. The validation workflow reuses the `.chunk/` frontend, Rust, and Playwright lane scripts so pre-push and outer-loop gates stay aligned. Alpha prereleases run from qualifying `main` pushes using calendar-semver technical versions (`YYYY.M.D-alpha.N`) and zero-padded GitHub tags (`alpha-vYYYY.M.D-alpha.NNNN`). Stable releases run from `vYYYY-MM-DD` and legacy `stable-vYYYY.M.D` tags. Both channels fan out signed macOS Apple Silicon/Intel, Linux x64, and Windows x64 builds; stable adds macOS DMGs. Windows always requires Tauri updater signatures and adds Authenticode plus `Get-AuthenticodeSignature` verification when certificate secrets are present. Linux uses Tauri's stock linuxdeploy AppImage output plugin and validates DEB/RPM desktop metadata plus installer and updater-signature artifacts. Downstream jobs assemble `alpha-latest.json` or `stable-latest.json`, publish GitHub Releases through the restricted release context, rebuild public download/history pages, and commit generated Pages output to `gh-pages`. macOS assets retain the `Tolaria_<version>_macOS_Silicon` and `Tolaria_<version>_macOS_Intel` base names. Packaged builds pass the computed version as `VITE_SENTRY_RELEASE`.
